...
(defun hermes-aux-now (args arglist)
...
(defun hermes-aux-tomatinho (mode)
  (with-temp-buffer
    (case mode
...
(defun hermes-aux-help (args arglist)
  (loop for x in (append hermes-commands hermes-custom-commands)
        ...
        (loop for h = y then (cddr h) while h
              unless (and hermes-hide-item (equal (car h) 'adult))
...
(defun hermes-aux-trans (args arglist)
...
(defun hermes-aux-freebase (status raw)
  (let* ((max 10)
    ...
    (loop for x in children and i upto max
          ...
          into collected doing (let* ((mid (cdr (assoc 'mid x))) (name (cdr (assoc 'name x)))
                                 ...
                                 (setq hermes-goodies (plist-put hermes-goodies i `(name ,name mid ,mid call (progn (browse-url ,url) "Opening page.")))))
          finally return (apply #'concat collected))))

(defun hermes-aux-reddit (status raw)
  (let* ((max 10)
         (json (json-read-from-string (html-entities-to-unicode (decode-coding-string (url-unhex-string raw) 'utf-8))))
    ...
    (loop for x in children and i upto max
          ...
          into collected doing (let ((url (cdr (assoc 'url x))) (title (cdr (assoc 'title x))))
                                 (setq hermes-goodies (plist-put hermes-goodies i `(name ,title call (progn (browse-url ,url) "Opening page.")))))
          finally return (apply #'concat collected))))

(defun hermes-aux-ddg (status html)
  (with-temp-buffer
    (insert html)
    (goto-char (point-min)) (keep-lines "a.*result-link") (flush-lines "/y.js")
    (setq hermes-goodies nil)
    ...
    (loop for x = (butlast (split-string (html-entities-to-unicode (decode-coding-string (url-unhex-string (buffer-string)) 'utf-8)) "\n"))
          ...
          into collected doing (let ((url (car x))) (setq hermes-goodies (plist-put hermes-goodies i `(name ,(car x) call (progn (browse-url ,url) "Opening page.")))))
          and finally return (apply #'concat collected))))

(defun hermes-aux-wa (status html)
  (with-temp-buffer
    (insert html)
    (goto-char (point-min)) (replace-regexp "<" "\n<")
...
(defun hermes-aux-au (status html)
  (with-temp-buffer
    (insert html)
    (goto-char (point-min))
...
(defun hermes-aux-mic (status html)
  (with-temp-buffer
    (insert html)
    (goto-char (point-min))
...
(defun hermes-aux-por (status html)
  (with-temp-buffer
    (insert html)
    (goto-char (point-min))
    (keep-lines "dolEntra")
...
(defun hermes-aux-vadio (status html)
  (with-temp-buffer
    (insert html)
...
(defun hermes-aux-latin (status html)
  (with-temp-buffer
    (insert html)
...
(defun hermes-aux-wordnet-call (a al)
  (hermes-async-command (format "http://wordnetweb.princeton.edu/perl/webwn?s=%s" (url-hexify-string a)) #'hermes-aux-wordnet))

(defun hermes-aux-wordnet (status html)
  (with-temp-buffer
    (insert html)
    (setq hermes-goodies nil)
    (goto-char (point-min))
    ...
    (loop for i = 0 then (1+ i) while (re-search-forward "<a href=\".*&amp;s=\\(.*\\)\">\\(.*\\)</a>" nil t)
          ...
          do (setq hermes-goodies (plist-put hermes-goodies i `(name ,(match-string 0) call (hermes-aux-wordnet-call ,(match-string 0) nil)))))
    (goto-char (point-min)) (replace-regexp "<[^>]*>" "")
    (buffer-string)))
...
(defun hermes-aux-tinycc (args arglist)
...
(defun hermes-aux-tinyurl (status html)
  (with-temp-buffer
    (insert html)
...
(provide 'hermes-commands)

;;; hermes-commands.el ends here
